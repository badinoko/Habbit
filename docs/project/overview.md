...
| HF-002 | REVIEW | Establish a reproducible local runtime baseline | Poetry deps installed, `postgres`/`redis` running, Alembic migrated, app responds on `http://127.0.0.1:8001/healthz/ready`; needs user validation and cleanup of local-only bootstrap artifacts |
