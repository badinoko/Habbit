...
from src.schemas.tasks import TaskCreateAPI, TaskInDB, TaskUpdateAPI
...
class DummyTaskRepo:
    ...
    async def add(self, data):  # type: ignore[no-untyped-def]
        return TaskInDB(
            id=uuid4(),
            name=data.name,
    ...
    async def update(self, task_id: UUID, data):  # type: ignore[no-untyped-def]
